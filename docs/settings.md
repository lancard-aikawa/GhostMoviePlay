...
`この機械の既定 (config.toml)` は完全に分かれていて、選んだ側だけを編集する。
入力欄には**その層が自分で持っている値だけ**が出る（空欄は「その層では未設定」で、
下に「いま効いている値」が出る）。

こうしてあるのは、行ごとに保存先を選ばせていたころ、機械の既定から来ている値を
**このプロジェクトだけ変えたつもりで既定（＝全プロジェクト）を書き換えていた**ため。
機械の既定を編集しているときは、保存先の行が赤くなる。

その中は**用途でタブを切り、行ごとに「値・由来」**を並べる。
`声と口調` / `何を撮るか` / `対象と動画` / `この機械` の 4 枚（その層に置けない
タブは出ない。プロジェクトを編集しているときに `この機械` は出ない）。
...
- その層に置けない項目は**そもそも出ない**（機械の既定に `app.url` は無い）
