...
- **チェックボックスとコンボは `select` で「その状態にする」。** `click` で
  切り替えると**前回の状態次第で結果が変わる** —— 7-Zip は「パスワードを表示」を
  憶えていて、2 回目の収録でチェックが外れ、**パスワードが伏字のまま撮れた**
- **`win=` でダイアログに乗り換える。** 1 つのアプリの操作はウィンドウ 1 つでは
  終わらない (7-Zip の圧縮ダイアログは別の exe の別ウィンドウ)。`Driver` は
  タイトルで掴み直すので、乗り換えても戻れる
- **前面に出せなかったら押さない。** `SetForegroundWindow` は呼び出し側が前面を
  持っていないと黙って失敗する。失敗したまま押すと**入力が別のアプリに入る**
  (実際に、打ったはずの文字がどこにも入らなかった)。前面のスレッドに
  `AttachThreadInput` してから頼み、それでも駄目なら落とす
- **`focus()` を毎回打たない。** 既に前面のときに `SetForegroundWindow` を
  打ち直すと、**ウィンドウの中の焦点が既定の部品に戻る** —— 入力欄を押してから
  `Control+a` を送ると、選ばれるのは一覧のファイル全部になる
- **範囲選択は修飾キー付きのクリック。** `Shift+Down` は相手によって効かない
  (7-Zip の一覧は焦点が動くだけ) —— 端を押して、もう一方の端を Shift 付きで押す
- **`RedrawWindow` で描き直させない。** 白紙で撮れるのを直そうとして入れたら、
  7-Zip 本体の**アドレスバーと状態バーが空になった**。撮る前に少し待つだけにする
- **絵が落ち着いてから撮る** (`record_windows.steady_shot` —— 同じ画が 2 回
  続いたら撮れたとみなす)。一覧の中身は「読めるようになった」あとから描かれるので、
  `wait_for` が通った直後に撮ると**行はあるのに描かれていない画**が残る
  (書庫の 4 行のうち 2 行しか写っていなかった)。**`byte 一致` では気づけない**
  —— 2 回撮ると同じところで同じように欠けるので、繰り返し撮れていることの確認を
  すり抜ける。**展示に出すサムネを見て初めて分かった**
