...
[lp-manager](https://github.com/lancard-aikawa/lp-manager) の説明動画 21 本での実測。
**実長は見積りの 1.8〜2.6 倍**になった。

| 内容 | 見積り | 実長 | 比 |
| --- | --- | --- | --- |
| 解説中心（画面を止めて喋る） | 39.1s | 100.2s | 2.6 |
| 入力と遷移が多い | 94.4s | 167.4s | 1.8 |
| 統計の読み上げ | 90.8s | 223.9s | 2.5 |

倍率が一定でないので、**見積りから実長を割り出すことはできない**。1 本に収まるか
どうかは撮ってから決めるしかない —— 実際、接続編（4分17秒）・効果測定（3分44秒）・
運用ナレッジ（3分20秒）はどれも撮ってから 2 本に割った。**割ると合計は伸びる**
（前置きと締めが 2 組要る）が、1 本あたりが 3 分を切って見返しやすくなる。
