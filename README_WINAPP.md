...
**Windows アプリでも、機械が操作して撮れることがある。** 相手が Win32 の部品で
できていれば、ビートに `actions` を書くだけで `gmp record` が通しで撮る
（人が撮る道と出るものは同じ）。**そのほうが必ず良い** —— 撮り直しが手作業で
なくなり、`gmp check` の腐敗検知も効く。

```bash
uv run gmp inspect                 # 開いているウィンドウの一覧
uv run gmp inspect "gmp\sample"    # 掴めるものと、一覧の行
```

掴めるものが出れば自動収録に進める（[docs/plan.md](docs/plan.md#windows-も機械に操作させられるwin32-の部品でできていれば)）。
`子ウィンドウがありません` と出たら、そのアプリ（WPF / WinUI / Electron）は
この方式では掴めないので、下の「人が操作して撮る」に進む。

**7-Zip は掴める。** UI Automation には Pane しか出さないので長く「届かない相手」
だったが、Win32 から見ると一覧もアドレス欄も揃って出る。一覧の行は
`row=給与明細_2026-07.pdf` のように**中身で指せる**ので、並び順が変わっても
同じところを押せる。
