...
| Android を自動で操作する | `android.py`（`Driver` と セレクタの解釈）、`record_android.py`（`SUPPORTED` と `do()` の分岐）、`cli.cmd_record` の分岐、`README_ANDROID.md` の自動収録、`docs/plan.md`、`tests/test_android.py` と `tests/test_record_android.py`。**実測で決まっていること**: Flutter はラベルを `text` ではなく `content-desc` に載せる（a-lamo で 35 ノード中 `text` は 0 件）／`uiautomator dump` は 1 回 2.6 秒なので**読んだら使い回す**／`app.setup` に **`am force-stop`** が要る（無いと前回の画面を引き継ぐ。**`pm clear` は使わない** —— ログインごと消えて `precondition` を自分で壊す）／`input text` は IME を通るので**ローマ字に読める小文字は変換される**（`kaigi` → かいぎ。大文字・数字・記号が混じれば素通し）|
