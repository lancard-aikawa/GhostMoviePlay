...
畳んでから**。`終了` を押さずに窓を閉じる人は必ずいるので、閉じるときも片付ける。
**仕込みは同期で走る**（画面が止まる）ので、数分かかるものを書かないこと。

**撮る窓を選び直しても `app.window` は書き換えない。** 1 つのアプリの操作は
**窓 1 つでは終わらない** —— 7-Zip なら圧縮ダイアログは別の exe (`7zG.exe`) の
別窓で、そこも撮る。毎回上書きすると、ダイアログを撮った拍子に「この 1 本の
対象」がダイアログになる。コンボは「いまどれを撮るか」、`app.window` は
「主な対象」で、**空のときだけ書く**。動画の大きさを窓に合わせるのも主な対象の
...
- `docs/video/assist-7zip/` —— **支援収録の唯一の実例**（人が操作して撮る）。
  ...
  撮る相手が本体とダイアログの**2 つの窓にまたがる**ので、窓を選び直す道が
  要ることもここで分かった
