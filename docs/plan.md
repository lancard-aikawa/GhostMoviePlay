...
**`actions` が全部通っても、目的を果たしたとは限らない。**

```jsonc
"goal": { "says": "満点 21 点で終わる", "selector": "#result",
          "contains": "21 点 (満点)" }   // absent: 含んでいてはいけない文字
```

`examples/demo` の盤面を釣り合いのために `[6,10,6,3,9,3]` から `[6,10,5,3,9,4]` へ
変えると、**セレクタは全部生きているのでクリックは通り、`gmp record` も
`gmp check` も緑のまま**、ナレーションだけが「6と6と9で、21点。満点です」と
嘘になる（実際にそうなった）。`goal` を書いてあれば赤になる。

- `contains` は「果たしたか」、`absent` は「**やってはいけないことをしていないか**」。
  後者が要るのは、**操作が成功していて結果だけが違う**ときに前者では捕まらないため
  （送信は通っていて宛先だけが全員、など）
...
#### Windows も機械に操作させられる（Win32 の部品でできていれば）

`app.window` の 1 本でも、ビートに `actions` を書くと `gmp record` が人の代わりに
操作して撮る。**撮り直しが手作業でなくなる**のが値打ちで、人が撮ると途中で状態が
変わったまま撮り足して「同じ画面のはずの 2 枚で表示が食い違う」ことが起きる。

**掴めるかは相手による。** UI Automation ではなく **Win32 のウィンドウツリー**を
見るので、子ウィンドウを持たないアプリ（WPF / WinUI / Electron）は掴めない。
先に確かめる:

```bash
uv run gmp inspect "gmp\sample"    # 掴めるものと、一覧の行が出る
```

セレクタは接頭辞つきで書く。**推測で書いたセレクタは収録時に必ず落ちる**ので、
`gmp inspect` に出たものから選ぶ。
...
| `class=Edit` | ウィンドウクラス |
| `cid=1001` | コントロール ID |
| `row=給与明細.pdf` / `row*=給与` | **一覧（`SysListView32`）の行を中身で指す** |
| `win=圧縮` | **別のウィンドウに乗り換える**（ダイアログ）。以降のセレクタはその中を見る |
| `at=0.5,0.75` | ウィンドウの中の割合（最後の手段） |

**1 つのアプリの操作はウィンドウ 1 つでは終わらない。** 7-Zip の圧縮ダイアログは
別の exe（`7zG.exe`）の別ウィンドウなので、`wait_for` に `win=圧縮` を書いて
乗り換える。閉じたら `win=<本体のタイトル>` で戻る。

**チェックボックスとコンボは `select` で「その状態にする」。** `click` で
切り替えると前回の状態次第で結果が変わり、**2 回目の収録で絵が変わる**
（7-Zip は「パスワードを表示」を憶えていて、実際にチェックが外れた）。

```jsonc
{ "type": "select", "selector": "cid=3803", "value": "on" }      // チェック
{ "type": "select", "selector": "cid=104",  "value": "zip" }     // コンボ（先頭一致）
```

**一覧の範囲選択は修飾キー付きのクリック。** `Shift+Down` で伸ばす手は相手に
よって効かない（7-Zip の一覧は焦点が動くだけだった）。端を押して、もう一方の端を
`"modifiers": "Shift"` 付きで押す。

**一覧は `row=` で指す。** 「上から 3 番目」は並べ替えやスクロールで別のものを
押すので、繰り返し撮れない。行は別プロセスからでも中身と矩形を読めるので、
**id が無い相手でも名前で指せる**（7-Zip がまさにそれ。UIA には Pane しか出さない）。

使えるのは `click` / `dblclick` / `hover` / `type` / `press` / `wait_for` /
`sleep` / `scroll_to` だけで、**それ以外は撮る前に落とす**。`type` は
**中身を置き換える**（アドレス欄のように最初から中身がある相手が普通なので）。
詳しくは [README_WINAPP.md](../README_WINAPP.md)。
...
- **`do` に「撮る人がやること」を書く。** `say` は観る人への言葉なので、それだけ
  ...
  ```jsonc
  { "do":  "ファイルマネージャー：sample.zip をダブルクリックして開く",
