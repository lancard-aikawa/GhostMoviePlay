...
SCENES_BLOCK = """scenes:
  - id: problem
    goal: 進む上で引っかかるところを実演する
  - id: why
    goal: 何がそうさせたかを画面を指しながら説明する
  - id: good
    goal: 正解ルートで通してみせる
---
  ...
  "scenes": [
    {
      "id": "fail-greedy",
      "title": "よくある問題",
      // **そのシーンが目的を果たしたか** (任意だが、書けるなら必ず書く)。
      //   selector の文字が contains を含む / absent を含まない、で見る。
      // **actions が全部通っても目的を果たしたとは限らない** —— 画面の数字が
      // 変わってもクリックは通るので、収録も検査も緑のまま説明だけが嘘になる。
      // 画面の状態が変わらない解説シーンには書かない
      "goal": { "says": "満点 21 点で終わる", "selector": "#result",
                "contains": "21 点 (満点)" },
      "beats": [
        {
          "say": "ナレーション原稿(音声用)",
...
- **代わりに `do` を必ず書いてください。** これは**撮る人への指示**で、動画には
  ...
  ```jsonc
  {
    "do":  "ファイルマネージャー：sample.zip をダブルクリックして開く",  // 撮る人へ
    "say": "ところが、開くとパスワードを訊かれません"                        // 観る人へ
  }
  ```

- **`do` は「どの画面か」から書き始めてください。** `<画面の名前>：<やること>` の形に
  そろえます。撮る人は上から順に読むので、**いまどの画面にいるはずかが書いて
  いないと、前のビートから移動したのかどうかが読めません** (実際に読めなかった)。
  画面の名前は**アプリが出しているものをそのまま**使うこと —— 勝手に言い換えると、
  撮る人が画面上のどれのことか分からなくなります。操作の要らない解説ビートも
  同じ形で、画面名のあとに「そのまま動かさない (説明)」と書きます。
...
- **どこにあるかまで書いてください。** 「追加を押す」ではなく
  ...
  探させてしまった)
- **前提は `do` に混ぜないでください。** 「ログイン済みであること」のような
  **始める前に満たしておくこと**は `app.precondition` の担当で、撮る画面が
  いちばん上に出します。手順に混ぜると、**1 枚目を撮ろうとして初めて詰まります**。
  依頼文に前提が書かれていないなら、`do` で補わずに**そのまま書かないでください**
  (勝手に手順へ足すと、毎回それを操作させることになります)
...
# **Windows は機械が撮れることがある。** 相手が Win32 の部品でできていれば、
# `actions` を書くと `gmp record` が通しで撮る (撮り直しが手作業でなくなる)。
# 掴めるかは相手によるので、**確かめる手順ごと**渡す —— 推測で書いたセレクタは
# 収録時に必ず落ちる (web で「Playwright MCP で触って DOM から取る」と同じ話)
WINDOWS_DRIVE_NOTE = """### 機械が操作して撮れるなら、そのほうがよい

このウィンドウが **Win32 の部品**でできているなら、`actions` を書けば
`gmp record` が通しで操作して撮ります。**人が撮ると撮り直しが全部手作業**になり、
途中で状態が変わったまま撮り足して「同じ画面のはずの 2 枚で表示が食い違う」ことも
起きます。機械が撮れるならそちらにしてください。

**まず確かめてください。** 掴めるかは相手によります (WPF / WinUI / Electron は
子ウィンドウを持たないので掴めません)。
...
掴めるものと、一覧の行がそのまま出ます。**出てきたものからセレクタを選んでください**
—— 推測で書いたセレクタは収録時に必ず落ちます。

| セレクタ | 意味 |
| --- | --- |
| `name=OK` / `name*=OK` | ウィンドウの文字 (ボタンのラベルなど) |
| `class=Edit` | ウィンドウクラス |
| `cid=1001` | コントロール ID |
...
- **画面が変わる操作のあとには `wait_for` を書いてください。** 待たずに次を
  押すと、前の画面の同じ場所を押します
- `actions` を書いたら **`do` は要りません** (操作するのは機械です)。
  掴めなかったときだけ、上の支援収録の書き方に戻してください
"""

GUIDE = """## Pass1 で守ること

1. **先にソースを読む。** ルール・スコア計算・勝敗条件を把握してから演目を組む。
   何が「良い手」かを理解していない状態で選んだ問題は、当てずっぽうになって教材にならない。
2. **問題を狙って作る。軸は「分からなくて離れてしまうか」。**
   撮る価値があるのは、**そこで人が諦めて離脱する**ところ。「これを知らないと詰む」
   という具体的な因果があるものを選ぶ。**失敗はその一種でしかない** —— 離脱は
   次のどれからでも起きる。

   - **失敗する** —— 操作が通らない、結果が壊れる
   - **通るのに目的を果たしていない** —— 操作は成功するが、意図した結果になっていない
   - **選べない・見つからない** —— 何をすればいいのか画面から決まらず、そこで止まる
   - **順序に縛られる** —— 先に別のことをしていないと進めない
   - **引き返せない** —— 進めるが、そこで何かが確定する

   迷ったら **「ここで人は離れるか」** で選ぶ。離れないなら、動画にするほどの問題では
   ない（ただの操作説明になる）。**分かっている人でも引っかかる**ものを選ぶこと ——
   ただ下手なだけの例は教材にならない。
...
def build_request(spec: Spec, resolved=None, plan_dir: str | Path | None = None) -> str:
    ...
    if window or package:
        ...
        extra = "\n" + ASSIST_SCHEMA_NOTE
        # **Windows は機械が撮れることがある。** Win32 の部品でできているアプリなら
