...
AI がアプリやゲームを実際に操作し、**人が分からなくて離れてしまうところとその理由、そして正解ルートまでを字幕付きで解説する動画**を生成するパイプライン。

Web で表示できるものなら何でも対象になる（ゲーム、業務アプリ、社内ツールのデモ）。
AI にプロジェクトフォルダを読ませるため、UI の外側から観察するのではなく
**仕様を理解した上で「人が分からなくて離れてしまうところを狙って作り」「なぜそうなるのかを説明できる」** のが特徴。
軸は離脱で、失敗はその一種。操作が全部通っても目的を果たしていなければ、同じだけ人は離れる。
...
- **plan.json は人間が読んで直せる。** AI の選んだ問題が的外れなら手で直せばよい。
...
scenes は「問題 → 何がそうさせたか → 正解ルート」の 3 幕で goal を書いて。
...
| `gmp inspect [タイトル]` | ウィンドウの中身を出す（Windows の自動収録でセレクタを選ぶ） |
