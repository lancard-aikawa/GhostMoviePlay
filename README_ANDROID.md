...
[README](README.md) — Android のアプリを**人が操作して撮る**ときの手引き。

**自動操作はしない。** 台本（`plan.json`）にシーンとビートの並び・`do`（撮る人が
やること）・`say`（観る人への言葉）が書いてあるので、それを上から読んで手を動かし、
ビートごとに画面を撮る。撮ったものを `gmp record` が並べ、`gmp render` が字幕と
音声を乗せる。
