...
**同じ行に置けるのは書ける層 (`layers`) が同じ設定だけ。** 書込先のドロップダウンは
行に 1 つなので、層の違うものを並べると選べる先が嘘になる（`series.count` は
プロジェクトにしか置けないので、`target_seconds` と同じ行にできない）。
`tests/test_ui.py` が見ている。
...
**書ける先が無い入力欄は出さない。** `gmp.toml` が無い状態で `app.url` の欄を
並べても、埋めた先が無い。`ui.visible()` が「書けるか / いま効いている値があるか」で
出し分ける（効いている値は直せなくても見せる価値がある）。
