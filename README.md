...
[README_WINAPP.md](README_WINAPP.md) の道は通っているが、**まだ 1 本しか撮っていない**
（[docs/video/assist-7zip/](docs/video/assist-7zip/)）。残っているのはこの 5 つ。

- **2 本目を撮る —— 相手は [Krita](https://krita.org/)**（`winget install KDE.Krita`）。
  1 本だけだと、その 1 本に合わせた作りになっていないかが分からない。7-Zip では
  **通らなかった経路が 2 つある**ので、そこを埋める相手を選んである。

  | | 7-Zip（1 本目） | Krita（2 本目） |
  | --- | --- | --- |
  | 録画 (clip) | **0 回**（9 ビート全部が静止画） | 筆を走らせるので**録画が要る** |
  | canvas / 独自描画 | 無し | **キャンバスは UIA から原理的に見えない** |

  題材は「**選択範囲を解除し忘れて描けない**」。一部を選択したあと解除せずに
  別の場所へ描くと筆が反応しない → 選択範囲の外は保護されている → 解除して描ける。
  **症状（描けない）と原因（さっきの選択）が離れている**ので、狙って失敗を作れる。
  アカウント不要・GPL なので公開しても困らない。

  **ログイン / OAuth は公開サンプルにできない** —— アカウント名や通知が画面に出る。
  やるなら手元だけの 1 本にして、リポジトリにも Gallery にも入れないこと
- **使っていないショットを数えて言う。** 撮り直すたびに `shots/` に貯まるが、どれが
  使われていないかは画面から分からない（1 本目で 15 枚中 6 枚が余った）。
