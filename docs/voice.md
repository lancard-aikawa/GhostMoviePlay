...
**ラテン文字の語は辞書では直せない。** 当たるのは日本語の表層形なので、
`a-lamo` のようなラテン文字 + ハイフンはトークナイザが割ってしまい、
「エイ、ラモ」と**読点が入って切れた声**になる。製品名は `say` のほうを
カタカナで書く —— 字幕は `subtitle` が別に持つので、見た目は変わらない。

**内蔵の読みが勝つ語がある。** `priority` の既定は 5。`gmp kana` で直って
いなければ `{ pronunciation = 'カラノママ', priority = 8 }` の形にする
（「空のまま」は 5 では「そらのまま」のままだった）。
