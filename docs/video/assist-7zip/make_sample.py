...
置き場所を `C:\gmp-sample` にしているのは、**7-Zip がタイトルバーとアドレスバーの
2 か所にパスを出す**から。深いところに置くと動画の上半分がパスで埋まるし、
`%USERPROFILE%` の下だと**ユーザー名が公開動画に写り込む**。
...
# **ドライブ直下に置く。** 7-Zip はタイトルバーとアドレスバーの 2 か所に
# パスを出すので、深いところや %USERPROFILE% の下だと動画の上半分が
# パスで埋まり、**ユーザー名まで写り込む** (公開する動画では消したい)
FOLDER = Path(os.environ.get("SystemDrive", "C:") + "/gmp-sample")
