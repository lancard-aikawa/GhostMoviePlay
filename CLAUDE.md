...
**仕込みが走る場所は撮る画面の `起動` しかない。** 支援収録では撮るのが人なので、
...
| 撮る価値の前提を変える | `spec.GUIDE` の 1・2 番（**2 番が「何を撮る価値があるか」を決める唯一の行**。3〜7 は前提に依らない品質規則）、`skills/ghostplay/SKILL.md` の description・価値の 1 行・手順 3、`README.md` の冒頭、`docs/ideas/README.md` の「芯にあるもの」、`docs/governance.md`、`tests/test_request.py`。**`video.md` の `scenes` は前提を変えない** —— goal を伝えるだけで、依頼文には GUIDE が無条件で入る。**「失敗」に狭めない** —— 操作が全部通っても目的を果たしていなければ同じだけ障害で、7-Zip の 1 本が実際にそれ（zip は正常に作られる）|
