...
撮り直しにしないため）。ただし黙って捨てると「通ったのだから合っている」と読めて
しまうので、`Recorder.warn()` が `{kind, where, message}` を溜めて `timing.json` の
`warnings` に書く。`gmp record --strict` はそれを終了コードにし、撮る面は収録の行に
件数を出す。録画開始の遅れが `video.leader` を超えたときの警告も同じ枠に入っている。
