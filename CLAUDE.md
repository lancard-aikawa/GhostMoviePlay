...
| 展示に出した動画の素材 | `lancard-aikawa/GhostMoviePlayGallery` の **`videos.py`**（カードの**再現コマンド**と素材へのリンク。`python build.py` が `index.html` と 1 本ぶんの `v/*.html` を吐くので、**HTML は手で直さない**）。素材を動かすと展示が黙って嘘になる（あちらは撮り直しても気づけない）。**mp4 はあちらにしか置かない** |
